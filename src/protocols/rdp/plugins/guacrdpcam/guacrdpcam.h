...
#ifndef GUAC_RDP_PLUGINS_GUACRDPCAM_H
#define GUAC_RDP_PLUGINS_GUACRDPCAM_H
...
struct guac_rdp_rdpcam_device_mapping;

/**
 * The name of the RDPCAM control/enumeration dynamic virtual channel.
 * This MUST match the MS-RDPCAM specification.
 */
#define GUAC_RDPCAM_CHANNEL_NAME "RDCamera_Device_Enumerator"
 ...
 */
#define GUAC_RDPCAM_DEVICE0_CHANNEL_NAME "RDCamera_Device_0"

/**
 * The name of the guacrdpcam plugin.
 */
#define GUAC_RDPCAM_PLUGIN_NAME "guacrdpcam"
 ...
 * to callbacks related to the receipt of new connections to the RDPCAM
 * channel.
 */
typedef struct guac_rdp_rdpcam_listener_callback {
    ...
    /** Back-reference to the RDPCAM plugin. */
    struct guac_rdp_rdpcam_plugin* plugin;

} guac_rdp_rdpcam_listener_callback;
 ...
 */
typedef struct guac_rdpcam_device {
    ...
    /**
     * Per-device frame stream for buffering video frames.
     * Independent queue for each device.
     */
    guac_rdpcam_stream* stream;

    /**
     * Per-device dequeue thread for encoding and transmitting frames.
     * Each device has its own thread reading from its own stream.
     */
 ...
 * RDPCAM channel, including receipt of data. 
 */
typedef struct guac_rdp_rdpcam_channel_callback {
    ...
    /**
     * The actual virtual channel instance along which the RDPCAM plugin
...
} guac_rdp_rdpcam_channel_callback;

/**
 * All data associated with Guacamole's RDPCAM plugin for FreeRDP.
 */
typedef struct guac_rdp_rdpcam_plugin {
    ...
    /**
     * The listener callback structure allocated when the RDPCAM plugin
     ...
     * Value: guac_rdpcam_device* (per-device state)
     * 
     ...
     */
    struct guac_rdp_rdpcam_device_mapping* device_id_mappings;
