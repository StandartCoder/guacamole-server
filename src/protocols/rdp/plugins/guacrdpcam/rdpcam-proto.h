...
#ifndef GUAC_RDP_PLUGINS_GUACRDPCAM_PROTO_H
#define GUAC_RDP_PLUGINS_GUACRDPCAM_PROTO_H
...
/*
 * RDPCAM protocol helpers.
 *
 * NOTE: These helpers provide a thin serialization layer for MS-RDPCAM
...
#define RDPCAM_VERSION_MAJOR 1u

/** Minor version we support (spec-compliant implementation target). */
#define RDPCAM_VERSION_MINOR 0u

/** Protocol version byte written in message headers (matches FreeRDP). */
#define RDPCAM_PROTO_VERSION 0x02u

/**
 * Temporary message type identifiers (to be replaced with MS-RDPCAM values
 * in a later step). Using named constants centralizes usage.
 */
/* Official MS-RDPCAM message IDs (mirroring FreeRDP's CAM_MSG_ID) */
typedef enum rdpcam_msg_type {
    RDPCAM_MSG_SUCCESS_RESPONSE            = 0x01,
    RDPCAM_MSG_ERROR_RESPONSE              = 0x02,
    RDPCAM_MSG_SELECT_VERSION_REQUEST      = 0x03,
    RDPCAM_MSG_SELECT_VERSION_RESPONSE     = 0x04,
    RDPCAM_MSG_DEVICE_ADDED_NOTIFICATION   = 0x05,
    RDPCAM_MSG_DEVICE_REMOVED_NOTIFICATION = 0x06,
    RDPCAM_MSG_ACTIVATE_DEVICE_REQUEST     = 0x07,
    RDPCAM_MSG_DEACTIVATE_DEVICE_REQUEST   = 0x08,
    RDPCAM_MSG_STREAM_LIST_REQUEST         = 0x09,
    RDPCAM_MSG_STREAM_LIST_RESPONSE        = 0x0A,
    RDPCAM_MSG_MEDIA_TYPE_LIST_REQUEST     = 0x0B,
    RDPCAM_MSG_MEDIA_TYPE_LIST_RESPONSE    = 0x0C,
    RDPCAM_MSG_CURRENT_MEDIA_TYPE_REQUEST  = 0x0D,
    RDPCAM_MSG_CURRENT_MEDIA_TYPE_RESPONSE = 0x0E,
    RDPCAM_MSG_START_STREAMS_REQUEST       = 0x0F,
    RDPCAM_MSG_STOP_STREAMS_REQUEST        = 0x10,
    RDPCAM_MSG_SAMPLE_REQUEST              = 0x11,
    RDPCAM_MSG_SAMPLE_RESPONSE             = 0x12,
    RDPCAM_MSG_SAMPLE_ERROR_RESPONSE       = 0x13,
    RDPCAM_MSG_PROPERTY_LIST_REQUEST       = 0x14,
    RDPCAM_MSG_PROPERTY_LIST_RESPONSE      = 0x15,
    RDPCAM_MSG_PROPERTY_VALUE_REQUEST      = 0x16,
    RDPCAM_MSG_PROPERTY_VALUE_RESPONSE     = 0x17,
    RDPCAM_MSG_SET_PROPERTY_VALUE_REQUEST  = 0x18
} rdpcam_msg_type;

/**
 * H.264 media subtype GUID used by MS-RDPCAM. This is the standard
 ...
 */
static const GUID RDPCAM_SUBTYPE_H264 =
    { 0x34363248, 0x0000, 0x0010, { 0x80, 0x00, 0x00, 0xAA, 0x00, 0x38, 0x9B, 0x71 } };

/**
 * Writes a SampleResponse header compliant with FreeRDP/MS-RDPCAM.
 ...
 *     Presentation timestamp in 100-ns units (HNS), per MS-RDPCAM.
 *
 * @return
 *     TRUE on success, FALSE on failure.
 */
BOOL rdpcam_write_sample_response_header(wStream* s,
 ...
 */
typedef struct rdpcam_media_type_desc {
 ...
 *     TRUE on success, FALSE on failure.
 */
BOOL rdpcam_build_version_request(wStream* s);
 ...
 *     TRUE on success, FALSE on failure.
 */
BOOL rdpcam_build_version_response(wStream* s);
 ...
 *     TRUE on success, FALSE on failure.
 */
BOOL rdpcam_build_success_response(wStream* s);
 ...
 *     TRUE on success, FALSE on failure.
 */
BOOL rdpcam_build_device_added(wStream* s, const char* device_name,
 ...
 *     TRUE on success, FALSE on failure.
 */
BOOL rdpcam_build_stream_list(wStream* s, const rdpcam_stream_desc* streams, size_t count);
 ...
 *     TRUE on success, FALSE on failure.
 */
BOOL rdpcam_build_media_type_list(wStream* s, const rdpcam_media_type_desc* media_types,
        size_t count);
 ...
 *     TRUE on success, FALSE on failure.
 */
BOOL rdpcam_build_current_media_type(wStream* s, const rdpcam_media_type_desc* media_type);
 ...
 */
void rdpcam_log_hex_dump(guac_client* client, const char* prefix,
 ...
 */
BOOL rdpcam_parse_sample_credits(const uint8_t* payload, size_t payload_len,
 ...
 *     TRUE on success, FALSE on parse error.
 */
BOOL rdpcam_parse_start_streams(const uint8_t* payload, size_t payload_len,
        uint8_t* out_stream_index, rdpcam_media_type_desc* out_media_type);
 ...
 *     TRUE on success, FALSE on parse error.
 */
BOOL rdpcam_parse_current_media_type_request(const uint8_t* payload, size_t payload_len,
 ...
 *     TRUE on success, FALSE on parse error.
 */
BOOL rdpcam_parse_media_type_list_request(const uint8_t* payload, size_t payload_len,
        uint8_t* out_stream_index);
 ...
 *     TRUE on success, FALSE on parse error.
 */
BOOL rdpcam_parse_sample_request(const uint8_t* payload, size_t payload_len,
        uint8_t* out_stream_index);
 ...
 */
BOOL rdpcam_parse_stop_streams(const uint8_t* payload, size_t payload_len);
 ...
 *     TRUE on success, FALSE on failure.
 */
BOOL rdpcam_build_start_streams_response(wStream* s, uint32_t status);
 ...
 *     TRUE on success, FALSE on failure.
 */
BOOL rdpcam_build_stop_streams_response(wStream* s, uint32_t status);
 ...
 *     TRUE on success, FALSE on failure.
 */
BOOL rdpcam_build_sample_error_response(wStream* s, uint8_t streamIndex);
 ...
 *     TRUE on success, FALSE on failure.
 */
BOOL rdpcam_build_device_removed(wStream* s, const char* channel_name);
