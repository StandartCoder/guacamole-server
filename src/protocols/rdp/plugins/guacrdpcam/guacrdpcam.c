...
#include "plugins/guacrdpcam/rdpcam-proto.h"
 ...
 */
#define GUAC_RDPCAM_SAMPLE_CREDITS 1u

#define GUAC_RDPCAM_DEFAULT_WIDTH 640u
#define GUAC_RDPCAM_DEFAULT_HEIGHT 480u
#define GUAC_RDPCAM_DEFAULT_FPS_NUM 30u
 ...
 */
static bool guac_rdp_rdpcam_should_hexdump(void) {
    static bool initialized = false;
    static bool enabled = false;
    if (!initialized) {
        const char* env = getenv("GUAC_RDPCAM_HEXDUMP");
        if (env && *env) {
 ...
 * @param channel_name
 *     The RDPCAM channel name associated with the payload.
 * @param channel_id
 ...
 */
static void guac_rdp_rdpcam_log_hexdump(guac_client* client, const char* direction,
        const char* channel_name, UINT32 channel_id, const BYTE* data, size_t length) {

    if (!client || !data || length == 0)
        return;

    if (!guac_rdp_rdpcam_should_hexdump())
        return;
    ...
    if (length > max_dump) {
        guac_client_log(client, GUAC_LOG_DEBUG,
                "RDPCAM %s %s[id=%" PRIu32 "] hexdump length=%zu truncated to %zu bytes",
                direction ? direction : "",
                channel_name ? channel_name : "rdpcam",
                channel_id,
                length,
                dump_len);
    }
    ...
    for (size_t offset = 0; offset < dump_len; offset += 16) {
        ...
        guac_client_log(client, GUAC_LOG_DEBUG,
                "RDPCAM %s %s[id=%" PRIu32 "] %04" PRIx64 "  %-48s %s",
                direction ? direction : "",
                channel_name ? channel_name : "rdpcam",
                channel_id,
                (uint64_t)offset,
...
static void guac_rdp_rdpcam_log_message(guac_client* client, const char* prefix,
    ...
    guac_client_log(client, GUAC_LOG_DEBUG,
            "RDPCAM %s %s[id=%" PRIu32 "] msg=0x%02X payload_len=%zu",
            prefix ? prefix : "", channel_name ? channel_name : "rdpcam",
            channel_id, cam_msg, payload_len);

    if (guac_rdp_rdpcam_should_hexdump())
        guac_rdp_rdpcam_log_hexdump(client, prefix, channel_name, channel_id, payload, payload_len);
}

static void guac_rdp_rdpcam_log_stream(guac_client* client, const char* prefix,
    ...
    guac_rdp_rdpcam_log_message(client, prefix, channel_name, channel_id,
            cam_msg, payload_len, payload);
}

/* Forward declarations for device helper functions (definitions at end of file) */
static guac_rdpcam_device* guac_rdpcam_device_create(
        guac_rdp_rdpcam_plugin* plugin, const char* device_name);
static void guac_rdpcam_device_destroy(guac_rdpcam_device* device);
static guac_rdpcam_device* guac_rdpcam_device_lookup(
        guac_rdp_rdpcam_plugin* plugin, const char* device_name);
static guac_rdp_rdpcam_device_caps* guac_rdp_rdpcam_get_device_caps(
        guac_rdp_client* rdp_client, const char* channel_name);
static UINT guac_rdp_rdpcam_new_connection(
        IWTSListenerCallback* listener_callback, IWTSVirtualChannel* channel,
        BYTE* data, int* accept,
        IWTSVirtualChannelCallback** channel_callback);

/**
 * Invoked when RDPCAM capabilities have been updated on the core side.
 ...
 */
void guac_rdp_rdpcam_caps_notify(guac_client* client) {
    if (!client)
        return;
    ...
    guac_rdp_rdpcam_plugin* plugin = rdp_client->rdpcam_plugin;
    if (!plugin || !plugin->version_negotiated || !plugin->enumerator_channel)
        return;
    
    guac_rwlock_acquire_write_lock(&(rdp_client->lock));

    if (!rdp_client->rdpcam_caps_updated) {
        guac_rwlock_release_lock(&(rdp_client->lock));
        return;
    }

    guac_client_log(client, GUAC_LOG_DEBUG,
            "RDPCAM caps_notify: processing capability update");

    /* Build set of new device IDs from capabilities */
    char* new_device_ids[GUAC_RDP_RDPCAM_MAX_DEVICES] = {NULL};
    unsigned int new_device_count = rdp_client->rdpcam_device_caps_count;

    guac_client_log(client, GUAC_LOG_DEBUG,
            "RDPCAM caps_notify: new capability count = %u", new_device_count);

    for (unsigned int i = 0; i < new_device_count; i++) {
        guac_rdp_rdpcam_device_caps* caps = &rdp_client->rdpcam_device_caps[i];
        if (caps->device_id) {
            new_device_ids[i] = caps->device_id;
            guac_client_log(client, GUAC_LOG_DEBUG,
                    "RDPCAM caps_notify: new device[%u] = '%s'", i, caps->device_id);
        }
    }
    ...
    for (unsigned int i = 0; i < remove_count; i++) {
        ...
        if (rs && rdpcam_build_device_removed(rs, channel_name)) {
            ...
            guac_client_log(client, GUAC_LOG_DEBUG,
                    "RDPCAM TX ChannelId=%" PRIu32 " MessageId=0x06 DeviceRemovedNotification (channel='%s') result=%u",
                    enum_channel_id, channel_name, result);
        }
        if (rs) Stream_Free(rs, TRUE);

        /* Clean up device structure */
        guac_rdpcam_device* device = (guac_rdpcam_device*)
    ...
    guac_client_log(client, GUAC_LOG_DEBUG,
            "RDPCAM caps_notify: starting device addition phase");
    ...
    if (new_device_count > 0) {
        ...
        guac_client_log(client, GUAC_LOG_DEBUG,
                "RDPCAM caps_notify: processing %u potential new devices", new_device_count);
        ...
        for (unsigned int i = 0; i < new_device_count; i++) {
            ...
            for (unsigned int check_idx = 0; check_idx < 100 && !found_slot; check_idx++) {
                ...
                if (plugin->devices) {
                    guac_rdpcam_device* existing_device = (guac_rdpcam_device*)
                        HashTable_GetItemValue(plugin->devices, channel_name);
                    if (existing_device) {
                        in_use = 1;
                    }
            ...
            if (!found_slot) {
                guac_client_log(client, GUAC_LOG_ERROR,
                        "RDPCAM no available channel slots for new device");
                continue;
            }

            guac_client_log(client, GUAC_LOG_DEBUG,
                    "RDPCAM assigning new device '%s' to channel '%s'",
            ...
            if (assigned_channel_idx > 0 && plugin->manager) {
                ...
                if (device_listener) {
                    ...
                    if (saved_channel_name) {
                        ...
                        device_listener->parent.OnNewChannelConnection = guac_rdp_rdpcam_new_connection;

                        plugin->manager->CreateListener(plugin->manager, channel_name, 0,
                                (IWTSListenerCallback*) device_listener, NULL);

                        guac_client_log(client, GUAC_LOG_DEBUG,
                                "RDPCAM registered listener for device channel: %s", channel_name);
                    } else {
            ...
            wStream* rs = Stream_New(NULL, 256);
            if (rs && rdpcam_build_device_added(rs, device_name, channel_name)) {
                Stream_SealLength(rs);
                const size_t out_len = Stream_Length(rs);
                ...
                guac_client_log(client, GUAC_LOG_DEBUG,
                        "RDPCAM TX ChannelId=%" PRIu32 " MessageId=0x05 DeviceAddedNotification (device='%s', channel='%s')",
        ...
        guac_client_log(client, GUAC_LOG_DEBUG,
                "RDPCAM capability update: added %u new device(s)", added_count);
    } else {
        guac_client_log(client, GUAC_LOG_DEBUG,
                "RDPCAM all cameras disabled");
    }

    rdp_client->rdpcam_caps_updated = 0;
    guac_rwlock_release_lock(&(rdp_client->lock));

    guac_client_log(client, GUAC_LOG_DEBUG,
            "RDPCAM caps_notify: completed capability update processing");
}
 ...
 *     Always NULL.
 */
static void* guac_rdp_rdpcam_send_camera_start_signal_callback(guac_user* user, void* data) {
 ...
 *     Always NULL.
 */
static void* guac_rdp_rdpcam_send_camera_stop_signal_callback(guac_user* user, void* data) {
...
/**
 * Dequeue thread entry point. Continuously pops frames from the rdpcam stream
 * and sends them to the RDP client via the RDPCAM protocol.
 *
 * @param arg
 *     Pointer to the guac_rdpcam_device structure for per-device frame processing.
 *
 * @return
 *     NULL on success.
 */
static void* guac_rdp_rdpcam_dequeue_thread(void* arg) {

    guac_rdpcam_device* device = (guac_rdpcam_device*) arg;
    if (!device || !device->stream) {
        return NULL;
    }
    
    guac_client* client = NULL;
    guac_rdpcam_stream* stream = device->stream;
    
    /* For logging, we need the client - get it from stream if available */
    ...
    guac_client_log(client, GUAC_LOG_DEBUG,
        "RDPCAM dequeue thread started for device: %s", device->device_name);
    ...
    while (true) {
        ...
        /* We have credits; attempt to pull a frame from the shared stream. */
        uint8_t* frame_data = NULL;
        size_t frame_length = 0;
        bool keyframe = false;
        uint32_t pts_ms = 0;
        if (!guac_rdpcam_pop(stream, &frame_data, &frame_length, &keyframe, &pts_ms)) {
        ...
        if (!frame_data || frame_length == 0) {
            guac_client_log(client, GUAC_LOG_WARNING, "RDPCAM received invalid frame data");
            if (frame_data) {
        ...
        if (!allow_send) {
            if (!stream_active) {
                guac_client_log(client, GUAC_LOG_DEBUG,
                    "RDPCAM dropping frame - streaming not active");
            }
            else if (!channel_available) {
                guac_client_log(client, GUAC_LOG_DEBUG,
                    "RDPCAM dropping frame - channel unavailable");
            }
            else if (waiting_for_keyframe && !keyframe) {
                guac_client_log(client, GUAC_LOG_DEBUG,
                    "RDPCAM dropping P-frame - waiting for keyframe to start stream");
            }
            else if (stop_requested) {
                guac_client_log(client, GUAC_LOG_DEBUG,
                    "RDPCAM dropping frame - device stopping");
            }
            guac_mem_free(frame_data);
            frames_dropped++;
            continue;
        ...
        /* Build RDPCAM sample (placeholder header + payload) */

        wStream* s = Stream_New(NULL, frame_length + 64);
        if (s && rdpcam_write_sample_response_header(s,
                    /*streamId*/ stream_idx,
            ...
            if (frames_processed < 8) {
                ...
                guac_client_log(client, GUAC_LOG_DEBUG,
                        "RDPCAM TX frame: pts_ms=%u -> pts_hns=%" PRIu64,
                        pts_ms, pts_hns);
            }
            ...
            pthread_mutex_unlock(&device->lock);
            guac_rdp_rdpcam_log_stream(client, "TX",
            ...
            if (result == CHANNEL_RC_OK) {
                ...
                guac_client_log(client, GUAC_LOG_DEBUG, "RDPCAM frame sent: %zu bytes, keyframe=%s, pts=%u ms, credits %u->%u",
                               frame_length, keyframe ? "yes" : "no", pts_ms, before, remaining);
                
            } else {
                /* DVC Write failed - log detailed information */
                guac_client_log(client, GUAC_LOG_WARNING, 
                    "RDPCAM DVC Write FAILED: size=%zu, result=0x%08X, frame=%zu, keyframe=%s", 
                ...
                if (result != CHANNEL_RC_OK) {
                    guac_client_log(client, GUAC_LOG_ERROR, 
                        "RDPCAM channel write failed (code=0x%08X), stopping streaming", result);
                    pthread_mutex_lock(&device->lock);
                    device->streaming = false;
            ...
            guac_client_log(client, GUAC_LOG_ERROR, "RDPCAM failed to build sample header");
        }
        ...
        if (frames_processed % 100 == 0 || (current_time - last_stats_time) >= 30) {
            ...
            pthread_mutex_unlock(&device->lock);
            guac_client_log(client, GUAC_LOG_INFO, "RDPCAM performance stats: device=%s, processed=%u, dropped=%u, drop_rate=%.1f%%, credits=%u, queue=%d/%d", 
                           device->device_name, frames_processed, frames_dropped, drop_rate, device_credits_log, 
                           guac_rdpcam_get_queue_size(stream), GUAC_RDPCAM_MAX_FRAMES);
    ...
    uint32_t total_frames = frames_processed + frames_dropped;
    float drop_rate = total_frames > 0 ? (float)frames_dropped * 100.0f / total_frames : 0.0f;
    guac_client_log(client, GUAC_LOG_INFO, "RDPCAM final stats for device=%s: processed=%u, dropped=%u, drop_rate=%.1f%%", 
                   device->device_name, frames_processed, frames_dropped, drop_rate);

    guac_client_log(client, GUAC_LOG_DEBUG, "RDPCAM dequeue thread stopped for device: %s", device->device_name);
...
/**
 * Processes a single RDPCAM protocol message delivered by FreeRDP. The
 ...
 */
static UINT guac_rdp_rdpcam_handle_data(guac_client* client, IWTSVirtualChannel* channel,
        wStream* stream, guac_rdp_rdpcam_channel_callback* rdpcam_channel_callback) {

    guac_rdp_client* rdp_client = (guac_rdp_client*) client->data;
    
    /* Resolve per-channel context supplied by FreeRDP. */
    guac_rdpcam_device* device = rdpcam_channel_callback ? rdpcam_channel_callback->device : NULL;
    guac_rdp_rdpcam_plugin* plugin = rdpcam_channel_callback ? rdpcam_channel_callback->plugin : NULL;
    const char* ch_name = rdpcam_channel_callback ? rdpcam_channel_callback->channel_name : "unknown";
    UINT32 channel_id = rdpcam_channel_callback ? rdpcam_channel_callback->channel_id : 0;
    ...
    if (data_length < 2) {
        guac_client_log(client, GUAC_LOG_WARNING, "RDPCAM message too short: %zu bytes (expected at least 2 for header)", data_length);
    ...
    /* Read MS-RDPCAM protocol header: [Version:1][MessageId:1] */
    ...
    guac_rdp_rdpcam_log_message(client, "RX", ch_name, channel_id, cam_msg,
            payload_len, payload_ptr);

    guac_client_log(client, GUAC_LOG_DEBUG, "RDPCAM RX message on %s[id=%" PRIu32 "]: version=%d, cam_msg=0x%02x, payload_len=%zu", 
                    ch_name, channel_id, version, cam_msg, data_length - 2);

    /* Verify protocol version */
    if (version != RDPCAM_PROTO_VERSION) {
        guac_client_log(client, GUAC_LOG_WARNING, 
            "RDPCAM received message with unexpected version: expected 0x%02x, got 0x%02x",
            RDPCAM_PROTO_VERSION, version);
        return CHANNEL_RC_OK;
    }
    ...
    /* Process message based on message ID */
    {
        ...
        switch (cam_msg) {
            case RDPCAM_MSG_SELECT_VERSION_RESPONSE: {
                /* Server accepted our version request */
                guac_client_log(client, GUAC_LOG_DEBUG,
                        "RDPCAM RX ChannelId=%" PRIu32 " MessageId=0x04 SelectVersionResponse (version=%d)",
                        channel_id, RDPCAM_PROTO_VERSION);
                ...
                if (plugin && !strcasecmp(ch_name, GUAC_RDPCAM_CHANNEL_NAME)) {
                ...
                unsigned int device_count = rdp_client->rdpcam_device_caps_count;
                guac_rwlock_release_lock(&(rdp_client->lock));
                
                if (device_count > 0 && plugin && plugin->enumerator_channel) {
                    guac_rwlock_acquire_write_lock(&(rdp_client->lock));
                    guac_rdp_rdpcam_send_device_notifications(plugin, client, rdp_client, plugin->enumerator_channel);
                    rdp_client->rdpcam_caps_updated = 0; /* Clear the flag */
                    guac_rwlock_release_lock(&(rdp_client->lock));
                } else {
                    guac_client_log(client, GUAC_LOG_DEBUG,
                            "RDPCAM version negotiated, waiting for device capabilities");
                }
                break;
            }

            case RDPCAM_MSG_ACTIVATE_DEVICE_REQUEST: {
                guac_client_log(client, GUAC_LOG_DEBUG, "RDPCAM received ActivateDeviceRequest on %s[id=%" PRIu32 "]", ch_name, channel_id);
                
                if (!strcasecmp(ch_name, GUAC_RDPCAM_CHANNEL_NAME)) {
                    rs = Stream_New(NULL, 8);
                    if (rs && rdpcam_build_success_response(rs)) {
                        Stream_SealLength(rs);
                        const size_t out_len = Stream_Length(rs);
                        guac_rdp_rdpcam_log_stream(client, "TX", ch_name, channel_id, rs);
                        pthread_mutex_lock(&(rdp_client->message_lock));
                        result = channel->Write(channel, (UINT32) out_len, Stream_Buffer(rs), NULL);
                        pthread_mutex_unlock(&(rdp_client->message_lock));
                        guac_client_log(client, GUAC_LOG_DEBUG,
                                "RDPCAM TX ChannelId=%" PRIu32 " MessageId=0x01 SuccessResponse (ActivateDevice control)",
                                channel_id);
                    }
                } else {
                    if (!device) {
                        guac_client_log(client, GUAC_LOG_WARNING,
                                "RDPCAM ActivateDevice on device channel but no device available (ChannelId=%" PRIu32 ")",
                                channel_id);
                        return CHANNEL_RC_OK;
                    }
                    ...
                    if (rs && rdpcam_build_success_response(rs)) {
                        ...
                        pthread_mutex_unlock(&(rdp_client->message_lock));
                        guac_client_log(client, GUAC_LOG_DEBUG,
                                "RDPCAM TX ChannelId=%" PRIu32 " MessageId=0x01 SuccessResponse (ActivateDevice device=%s)",
            ...
            case RDPCAM_MSG_DEACTIVATE_DEVICE_REQUEST: {
                guac_client_log(client, GUAC_LOG_DEBUG, "RDPCAM received DeactivateDeviceRequest on %s[id=%" PRIu32 "]", ch_name, channel_id);
                ...
                if (!strcasecmp(ch_name, GUAC_RDPCAM_CHANNEL_NAME)) {
                    ...
                    if (rs && rdpcam_build_success_response(rs)) {
                        ...
                        pthread_mutex_unlock(&(rdp_client->message_lock));
                        guac_client_log(client, GUAC_LOG_DEBUG,
                                "RDPCAM TX ChannelId=%" PRIu32 " MessageId=0x01 SuccessResponse (DeactivateDevice control)",
                                channel_id);
                    }
                } else {
                    if (!device || !device->stream) {
                        guac_client_log(client, GUAC_LOG_WARNING,
                                "RDPCAM DeactivateDevice on device channel but no device/stream available (ChannelId=%" PRIu32 ")",
                                channel_id);
                        return CHANNEL_RC_OK;
                    }

                    bool same_stream_channel = (device->stream_channel == channel) || rdpcam_channel_callback->is_stream_channel;
                    if (!same_stream_channel) {
                        ...
                        if (rs && rdpcam_build_success_response(rs)) {
                            ...
                            pthread_mutex_unlock(&(rdp_client->message_lock));
                            guac_client_log(client, GUAC_LOG_DEBUG,
                                    "RDPCAM TX ChannelId=%" PRIu32 " MessageId=0x01 SuccessResponse (DeactivateDevice property device=%s)",
                                    channel_id, device->device_name);
                        }
                        break;
                    }

                    rdpcam_channel_callback->is_stream_channel = true;
                    ...
                    guac_client_log(client, GUAC_LOG_DEBUG,
                            "RDPCAM DeactivateDevice device=%s was_active_sender=%s",
                            device->device_name, was_active_sender ? "true" : "false");

                    guac_rdpcam_stream* stream = device->stream;

                    /* Only clear this device's stream state if it was the active sender.
                     * If this device was never streaming, its stream state is already clean.
                     * Only the active sender has stream state that needs to be cleared. */
                    if (was_active_sender) {
                         ...
                         * The blob handler will safely drop frames when rdpcam_stream is NULL. */
                        if (rdp_client->rdpcam_stream == stream)
                            rdp_client->rdpcam_stream = NULL;

                        guac_client_log(client, GUAC_LOG_DEBUG,
                                "RDPCAM cleared shared stream state for device=%s",
                                device->device_name);
                    } else {
                        guac_client_log(client, GUAC_LOG_DEBUG,
                                "RDPCAM NOT clearing shared stream state (device=%s was not active sender)",
                                device->device_name);
                    }
                    ...
                    for (uint32_t i = 0; i < outstanding; i++) {
                        ...
                        if (es && rdpcam_build_sample_error_response(es, stream_index)) {
                            Stream_SealLength(es);
                            const size_t err_len = Stream_Length(es);
                            guac_rdp_rdpcam_log_stream(client, "TX", ch_name, channel_id, es);
                            pthread_mutex_lock(&(rdp_client->message_lock));
                            channel->Write(channel, (UINT32) err_len, Stream_Buffer(es), NULL);
                            pthread_mutex_unlock(&(rdp_client->message_lock));
                            guac_client_log(client, GUAC_LOG_DEBUG,
                                    "RDPCAM TX ChannelId=%" PRIu32 " MessageId=0x13 SampleErrorResponse (stream=%u)",
                    ...
                    if (rs && rdpcam_build_success_response(rs)) {
                        ...
                        pthread_mutex_unlock(&(rdp_client->message_lock));
                        guac_client_log(client, GUAC_LOG_DEBUG,
                                "RDPCAM TX ChannelId=%" PRIu32 " MessageId=0x01 SuccessResponse (DeactivateDevice device=%s)",
                                channel_id, device->device_name);
                        ...
                        if (was_active_sender) {
                            guac_client_log(client, GUAC_LOG_DEBUG,
                                    "RDPCAM sending camera-stop to browser (device %s was active sender)",
                                    device->device_name);
                            guac_client_for_owner(client,
                                guac_rdp_rdpcam_send_camera_stop_signal_callback,
                                NULL);
                        } else {
                            guac_client_log(client, GUAC_LOG_DEBUG,
                                    "RDPCAM NOT sending camera-stop to browser (device %s was not active sender)",
                                    device->device_name);
                        }
                    }
                }
            ...
            case RDPCAM_MSG_STREAM_LIST_REQUEST: {
                if (!device) {
                    guac_client_log(client, GUAC_LOG_WARNING,
                            "RDPCAM StreamListRequest received but no device available on ChannelId=%" PRIu32,
                            channel_id);
                    return CHANNEL_RC_OK;
                }
                
                /* StreamListRequest has no payload - just respond with our stream list */
                guac_client_log(client, GUAC_LOG_DEBUG, "RDPCAM received StreamListRequest");
                
                rdpcam_stream_desc stream = {
                ...
                if (rs && rdpcam_build_stream_list(rs, &stream, 1)) {
                    Stream_SealLength(rs);
                    const size_t out_len = Stream_Length(rs);
                    guac_rdp_rdpcam_log_stream(client, "TX", ch_name, channel_id, rs);
                    pthread_mutex_lock(&(rdp_client->message_lock));
                    ...
                    pthread_mutex_unlock(&(rdp_client->message_lock));
                    guac_client_log(client, GUAC_LOG_DEBUG,
                            "RDPCAM TX ChannelId=%" PRIu32 " MessageId=0x0A StreamListResponse (streams=%u)",
                            channel_id, 1U);
                }
                break;
            }

            case RDPCAM_MSG_MEDIA_TYPE_LIST_REQUEST: {
                if (!device) {
                    guac_client_log(client, GUAC_LOG_WARNING,
                            "RDPCAM MediaTypeListRequest received but no device available on ChannelId=%" PRIu32,
                            channel_id);
                    return CHANNEL_RC_OK;
                }
                ...
                if (payload_len < 1) {
                    guac_client_log(client, GUAC_LOG_WARNING,
                            "RDPCAM MediaTypeListRequest missing stream index (payload_len=%zu) ChannelId=%" PRIu32,
                ...
                guac_client_log(client, GUAC_LOG_DEBUG, "RDPCAM received MediaTypeListRequest for stream %d", stream_idx);
                
                rdpcam_media_type_desc media_types[GUAC_RDP_RDPCAM_MAX_FORMATS];
                size_t media_type_count = 0;

                /* Get formats for this specific device */
                guac_rwlock_acquire_read_lock(&(rdp_client->lock));
                guac_rdp_rdpcam_device_caps* caps = NULL;
                if (ch_name) {
                    caps = guac_rdp_rdpcam_get_device_caps(rdp_client, ch_name);
                }

                if (caps && caps->format_count > 0) {
                    /* Use formats from this device's capabilities */
                    for (unsigned int i = 0; i < caps->format_count
                            && media_type_count < GUAC_RDP_RDPCAM_MAX_FORMATS; i++) {
                        guac_rdp_rdpcam_format* fmt = &caps->formats[i];
                        if (!fmt->width || !fmt->height || !fmt->fps_num)
                            continue;
                        media_types[media_type_count++] = (rdpcam_media_type_desc) {
                ...
                if (media_type_count == 0) {
                    media_types[media_type_count++] = (rdpcam_media_type_desc) {
                        .Format = CAM_MEDIA_FORMAT_H264,
                        .Width = GUAC_RDPCAM_DEFAULT_WIDTH,
                        .Height = GUAC_RDPCAM_DEFAULT_HEIGHT,
                        .FrameRateNumerator = GUAC_RDPCAM_DEFAULT_FPS_NUM,
                        .FrameRateDenominator = GUAC_RDPCAM_DEFAULT_FPS_DEN,
                        .PixelAspectRatioNumerator = 1,
                        .PixelAspectRatioDenominator = 1,
                        .Flags = CAM_MEDIA_TYPE_DESCRIPTION_FLAG_DecodingRequired
                    };
                ...
                if (rs && rdpcam_build_media_type_list(rs, media_types, media_type_count)) {
                    Stream_SealLength(rs);
                    const size_t out_len = Stream_Length(rs);
                    guac_rdp_rdpcam_log_stream(client, "TX", ch_name, channel_id, rs);
                    pthread_mutex_lock(&(rdp_client->message_lock));
                    ...
                    pthread_mutex_unlock(&(rdp_client->message_lock));
                    guac_client_log(client, GUAC_LOG_DEBUG,
                            "RDPCAM TX ChannelId=%" PRIu32 " MessageId=0x0C MediaTypeListResponse (count=%zu)",
                            channel_id, media_type_count);
                }
                break;
            }

            case RDPCAM_MSG_CURRENT_MEDIA_TYPE_REQUEST: {
                if (!device) {
                    guac_client_log(client, GUAC_LOG_WARNING,
                            "RDPCAM CurrentMediaTypeRequest received but no device available on ChannelId=%" PRIu32,
                            channel_id);
                    return CHANNEL_RC_OK;
                }
                ...
                if (payload_len < 1) {
                    guac_client_log(client, GUAC_LOG_WARNING,
                            "RDPCAM CurrentMediaTypeRequest missing stream index (ChannelId=%" PRIu32 ")",
                            channel_id);
                    return CHANNEL_RC_OK;
                }
                uint8_t stream_idx;
                ...
                if (stream_idx == 0) {
                    ...
                    rdpcam_media_type_desc media_type = device->media_type;
                    if (media_type.Format == 0) {
                        ...
                        if (caps && caps->format_count > 0) {
                            ...
                            guac_rdp_rdpcam_format* preferred = &caps->formats[0];
                            media_type = (rdpcam_media_type_desc){
                                .Format = CAM_MEDIA_FORMAT_H264,
                    ...
                    if (rs && rdpcam_build_current_media_type(rs, &media_type)) {
                        Stream_SealLength(rs);
                        const size_t out_len = Stream_Length(rs);
                        guac_rdp_rdpcam_log_stream(client, "TX", ch_name, channel_id, rs);
                        pthread_mutex_lock(&(rdp_client->message_lock));
                        ...
                        pthread_mutex_unlock(&(rdp_client->message_lock));
                        guac_client_log(client, GUAC_LOG_DEBUG,
                                "RDPCAM TX ChannelId=%" PRIu32 " MessageId=0x0E CurrentMediaTypeResponse (format=%u, %ux%u@%u/%u)",
            ...
            case RDPCAM_MSG_START_STREAMS_REQUEST: {
                if (!device || !device->stream) {
                    guac_client_log(client, GUAC_LOG_WARNING,
                            "RDPCAM StartStreamsRequest received but no device/stream available (ChannelId=%" PRIu32 ")",
                            channel_id);
                    return CHANNEL_RC_OK;
                }
                ...
                if (payload_len < 1 + 26) {  /* stream_idx + media_type_desc */
                    guac_client_log(client, GUAC_LOG_WARNING, "RDPCAM StartStreamsRequest too short");
                    return CHANNEL_RC_OK;
                }
                ...
                rdpcam_media_type_desc media_type;
                ...
                if (rdp_client->rdpcam_stream != NULL && rdp_client->rdpcam_stream != device->stream) {
                    guac_rdpcam_stream* old_stream = rdp_client->rdpcam_stream;

                    guac_client_log(client, GUAC_LOG_DEBUG,
                            "RDPCAM switching cameras: stopping previous device to start %s",
                            device->device_name);

                    /* Find and stop the old device that owns old_stream */
                    if (plugin && plugin->devices) {
                        ...
                        for (unsigned int check_idx = 0; check_idx < 100; check_idx++) {
                            ...
                            guac_rdpcam_device* old_device =
                                (guac_rdpcam_device*) HashTable_GetItemValue(plugin->devices, (void*) check_channel);
                            if (old_device && old_device->stream == old_stream) {
                                /* Stop the old device */
                                ...
                                guac_client_log(client, GUAC_LOG_DEBUG,
                                        "RDPCAM stopped streaming on device %s for camera switch",
                    ...
                    /* Clear the old stream's streaming state */
                    pthread_mutex_lock(&old_stream->lock);
                    old_stream->streaming = false;
                    old_stream->credits = 0;
                    old_stream->has_active_sender = false;
                    old_stream->active_sender_channel = NULL;
                    pthread_mutex_unlock(&old_stream->lock);
                    ...
                    guac_client_log(client, GUAC_LOG_DEBUG,
                            "RDPCAM stopped streaming on old device, proceeding with %s (browser will be notified via camera-start)",
                            device->device_name);
                }
                ...
                if (stream_idx == 0) {
                    ...
                    int flushed = 0;
                    while (stream->queue_head) {
                        guac_rdpcam_frame* stale = stream->queue_head;
                        stream->queue_head = stale->next;
                    ...
                    if (flushed > 0) {
                        guac_client_log(client, GUAC_LOG_DEBUG,
                            "RDPCAM flushed %d stale frames before streaming", flushed);
                    }

                    stream->stopping = false;
                    stream->streaming = true;
                    stream->credits = 0;
                    stream->stream_index = stream_idx;
                    if (!stream->has_active_sender) {
                        stream->has_active_sender = true;
                        stream->active_sender_channel = (void*) channel;
                        guac_client_log(client, GUAC_LOG_DEBUG,
                            "RDPCAM active sender claimed by device channel");
                    }
                    pthread_mutex_unlock(&stream->lock);

                    guac_client_log(client, GUAC_LOG_DEBUG,
                        "RDPCAM streaming started ChannelId=%" PRIu32 " format=%u %ux%u@%u/%u",
                        channel_id, media_type.Format, media_type.Width, media_type.Height,
                        media_type.FrameRateNumerator, media_type.FrameRateDenominator);

                    /* Browser pushes frames into rdp_client->rdpcam_stream. Point it at this device's stream. */
                    rdp_client->rdpcam_stream = stream;

                    rs = Stream_New(NULL, 8);
                    if (rs && rdpcam_build_start_streams_response(rs, 0)) {
                        Stream_SealLength(rs);
                        const size_t out_len = Stream_Length(rs);
                        guac_rdp_rdpcam_log_stream(client, "TX", ch_name, channel_id, rs);
                        pthread_mutex_lock(&(rdp_client->message_lock));
                        ...
                        pthread_mutex_unlock(&(rdp_client->message_lock));
                        guac_client_log(client, GUAC_LOG_DEBUG,
                                "RDPCAM TX ChannelId=%" PRIu32 " MessageId=0x01 SuccessResponse (StartStreams)",
                                channel_id);
                        ...
                        if (result == CHANNEL_RC_OK) {
                            ...
                            guac_client_for_owner(client,
                                guac_rdp_rdpcam_send_camera_start_signal_callback,
                                &camera_params);

                            guac_client_log(client, GUAC_LOG_DEBUG,
                                "RDPCAM sent camera-start signal to JavaScript: "
            ...
            case RDPCAM_MSG_STOP_STREAMS_REQUEST: {
                if (!device || !device->stream) {
                    guac_client_log(client, GUAC_LOG_WARNING,
                            "RDPCAM StopStreamsRequest received but no device/stream available (ChannelId=%" PRIu32 ")",
                            channel_id);
                    return CHANNEL_RC_OK;
                }
                ...
                rs = Stream_New(NULL, 8);
                if (rs && rdpcam_build_stop_streams_response(rs, /*status*/ 0)) {
                    Stream_SealLength(rs);
                    const size_t out_len = Stream_Length(rs);
                    guac_rdp_rdpcam_log_stream(client, "TX", ch_name, channel_id, rs);
                    pthread_mutex_lock(&(rdp_client->message_lock));
                    ...
                    pthread_mutex_unlock(&(rdp_client->message_lock));
                    guac_client_log(client, GUAC_LOG_DEBUG,
                            "RDPCAM TX ChannelId=%" PRIu32 " MessageId=0x01 SuccessResponse (StopStreams)",
                            channel_id);
                    ...
                    if (result == CHANNEL_RC_OK) {
                        ...
                        guac_client_log(client, GUAC_LOG_DEBUG, 
                            "RDPCAM sent camera-stop signal to JavaScript");
                    }
                }
                break;
            }

            case RDPCAM_MSG_PROPERTY_LIST_REQUEST: {
                if (!device) {
                    guac_client_log(client, GUAC_LOG_WARNING,
                            "RDPCAM PropertyListRequest received but no device available (ChannelId=%" PRIu32 ")",
                            channel_id);
                    return CHANNEL_RC_OK;
                }
                ...
                guac_client_log(client, GUAC_LOG_DEBUG,
                        "RDPCAM received PropertyListRequest ChannelId=%" PRIu32, channel_id);
                
                rs = Stream_New(NULL, 8);
                if (rs) {
                    Stream_Write_UINT8(rs, RDPCAM_PROTO_VERSION);
                    Stream_Write_UINT8(rs, RDPCAM_MSG_PROPERTY_LIST_RESPONSE);
                    Stream_SealLength(rs);
                    const size_t out_len = Stream_Length(rs);
                    guac_rdp_rdpcam_log_stream(client, "TX", ch_name, channel_id, rs);
                    pthread_mutex_lock(&(rdp_client->message_lock));
                    ...
                    pthread_mutex_unlock(&(rdp_client->message_lock));
                    guac_client_log(client, GUAC_LOG_DEBUG,
                            "RDPCAM TX ChannelId=%" PRIu32 " MessageId=0x15 PropertyListResponse (empty)",
                            channel_id);
                }
                break;
            }

            case RDPCAM_MSG_SAMPLE_REQUEST: {
                if (!device) {
                    guac_client_log(client, GUAC_LOG_WARNING, "RDPCAM SampleRequest received but no device available on %s[id=%" PRIu32 "]", ch_name, channel_id);
                    return CHANNEL_RC_OK;
                }

                /* Read stream index */
                if (payload_len < 1) {
                    guac_client_log(client, GUAC_LOG_WARNING, "RDPCAM SampleRequest missing stream index");
                    return CHANNEL_RC_OK;
                }
                uint8_t stream_idx;
                Stream_Read_UINT8(stream, stream_idx);
                ...
                if (stream_idx == 0 || stream_idx == device->stream_index) {
                    ...
                    rdpcam_channel_callback->is_stream_channel = true;
                    uint32_t before = device->credits;
                    device->credits = GUAC_RDPCAM_SAMPLE_CREDITS;
                    uint32_t remaining = device->credits;
                    /* Wake dequeue thread waiting on this device */
                    pthread_cond_broadcast(&device->credits_signal);
                    pthread_mutex_unlock(&device->lock);

                    /* Ensure browser has a stream to push into if streaming is active */
                    if (device->streaming) {
                        guac_rdp_client* rdp_client_ctx = (guac_rdp_client*) client->data;
                        if (rdp_client_ctx && !rdp_client_ctx->rdpcam_stream) {
                            rdp_client_ctx->rdpcam_stream = device->stream;
                            guac_client_log(client, GUAC_LOG_DEBUG,
                                "RDPCAM bound session stream to active device due to SampleRequest (channel=%" PRIu32 ")",
                                channel_id);
                        }
                    }
                    int queue_size = guac_rdpcam_get_queue_size(device->stream);
                    guac_client_log(client, GUAC_LOG_DEBUG,
                                   "RDPCAM SampleRequest ChannelId=%" PRIu32 " device=%s credits %u->%u queue=%d/%d",
                                   channel_id, device->device_name, before, remaining, queue_size,
                                   GUAC_RDPCAM_MAX_FRAMES);
                }
                break;
            }
    ...
    if (plugin && plugin->version_negotiated && plugin->enumerator_channel) {
        guac_rwlock_acquire_write_lock(&(rdp_client->lock));
        if (rdp_client->rdpcam_caps_updated && rdp_client->rdpcam_device_caps_count > 0) {
            guac_rdp_rdpcam_send_device_notifications(plugin, client, rdp_client, plugin->enumerator_channel);
            rdp_client->rdpcam_caps_updated = 0; /* Clear the flag */
            guac_client_log(client, GUAC_LOG_DEBUG,
                    "RDPCAM sent device notifications after late capability update");
        }
        guac_rwlock_release_lock(&(rdp_client->lock));
    }
...
/**
 * Callback which is invoked when data is received along the RDPCAM channel.
 * This callback is API-dependent and delegates to the API-independent
 * guac_rdp_rdpcam_handle_data function.
 *
 ...
 *     Always zero.
 */
static UINT guac_rdp_rdpcam_data(IWTSVirtualChannelCallback* channel_callback,
        wStream* stream) {

    guac_rdp_rdpcam_channel_callback* rdpcam_channel_callback =
        (guac_rdp_rdpcam_channel_callback*) channel_callback;
    IWTSVirtualChannel* channel = rdpcam_channel_callback->channel;

    /* Invoke generalized (API-independent) data handler with full callback context */
    guac_rdp_rdpcam_handle_data(rdpcam_channel_callback->client, channel, stream, rdpcam_channel_callback);

    return CHANNEL_RC_OK;
}

/**
 * Callback which is invoked when the RDPCAM channel is opened.
 ...
 */
static UINT guac_rdp_rdpcam_open(IWTSVirtualChannelCallback* channel_callback) {

    guac_rdp_rdpcam_channel_callback* rdpcam_channel_callback =
        (guac_rdp_rdpcam_channel_callback*) channel_callback;
    IWTSVirtualChannel* channel = rdpcam_channel_callback->channel;
    guac_client* client = rdpcam_channel_callback->client;
    const char* ch_name = rdpcam_channel_callback->channel_name;
    const UINT32 channel_id = rdpcam_channel_callback ? rdpcam_channel_callback->channel_id : 0;

    guac_client_log(client, GUAC_LOG_DEBUG, "RDPCAM channel opened (%s) [id=%" PRIu32 "]", ch_name,
            channel_id);

    /* On control channel: initiate version negotiation */
    if (strcasecmp(ch_name, GUAC_RDPCAM_CHANNEL_NAME) == 0) {
        wStream* s = Stream_New(NULL, 8);
        if (s && rdpcam_build_version_request(s)) {
            Stream_SealLength(s);
            const size_t out_len = Stream_Length(s);
            guac_rdp_rdpcam_log_stream(client, "TX", ch_name, channel_id, s);
            ...
            guac_rdp_rdpcam_channel_callback* cb = (guac_rdp_rdpcam_channel_callback*) channel_callback;
            if (cb && cb->plugin) {
                guac_rdp_rdpcam_plugin* plugin = cb->plugin;
                plugin->enumerator_channel = channel;
            }
        }
        else {
...
/**
 * Callback which is invoked when a connection to the RDPCAM channel is
 * closed.
 *
 * @param channel_callback
 ...
 *     Always zero.
 */
static UINT guac_rdp_rdpcam_close(IWTSVirtualChannelCallback* channel_callback) {

    guac_rdp_rdpcam_channel_callback* rdpcam_channel_callback =
        (guac_rdp_rdpcam_channel_callback*) channel_callback;
    guac_rdpcam_device* device = rdpcam_channel_callback->device;
    guac_rdp_rdpcam_plugin* plugin = rdpcam_channel_callback->plugin;
    const char* ch_name = rdpcam_channel_callback->channel_name;

    /* Log channel close */
    guac_client_log(rdpcam_channel_callback->client, GUAC_LOG_DEBUG,
            "RDPCAM channel connection closed (%s) [id=%" PRIu32 "]",
            rdpcam_channel_callback->channel_name ? rdpcam_channel_callback->channel_name : "unknown",
            rdpcam_channel_callback->channel_id);
    ...
    if (device) {
        ...
        if (device->stream_channel == rdpcam_channel_callback->channel)
            closing_stream_channel = true;
        else if (rdpcam_channel_callback->is_stream_channel)
            closing_stream_channel = true;
        ...
        if (closing_stream_channel) {
            guac_rdpcam_signal_stop(device->stream);
            ...
            if (was_active_sender) {
                guac_client_log(rdpcam_channel_callback->client, GUAC_LOG_DEBUG,
                        "RDPCAM sending camera-stop to browser (stream channel closed for active sender device %s)",
                        ch_name ? ch_name : "unknown");
                guac_client_for_owner(rdpcam_channel_callback->client,
                    guac_rdp_rdpcam_send_camera_stop_signal_callback,
                    NULL);
            } else {
                guac_client_log(rdpcam_channel_callback->client, GUAC_LOG_DEBUG,
                        "RDPCAM NOT sending camera-stop to browser (stream channel closed for non-active device %s)",
                        ch_name ? ch_name : "unknown");
            }

            guac_rdp_client* close_rdp_client =
                (guac_rdp_client*) rdpcam_channel_callback->client->data;
            if (close_rdp_client && close_rdp_client->rdpcam_stream == device->stream)
                close_rdp_client->rdpcam_stream = NULL;
        }
        ...
        if (plugin && plugin->devices && ch_name && remaining_refs == 0) {
            ...
            guac_rdpcam_device* to_destroy = device;
            if (HashTable_Remove(plugin->devices, (void*) ch_name)) {
                guac_client_log(rdpcam_channel_callback->client, GUAC_LOG_DEBUG,
                    "RDPCAM device removed from registry: %s", ch_name);
                guac_rdpcam_device_destroy(to_destroy);
            }
        }
        else if (remaining_refs != 0) {
            guac_client_log(rdpcam_channel_callback->client, GUAC_LOG_DEBUG,
                "RDPCAM device %s still referenced (%d), deferring destruction",
                ch_name ? ch_name : "unknown", remaining_refs);
        }
    }

    /* Free channel callback */
    guac_mem_free(rdpcam_channel_callback);
...
/**
 * Callback which is invoked when a new connection to the RDPCAM channel is
 ...
 *     The listener callback structure that was registered for the RDPCAM
 *     channel.
 *
 * @param channel
...
static UINT guac_rdp_rdpcam_new_connection(
    ...
    guac_rdp_rdpcam_listener_callback* rdpcam_listener_callback =
        (guac_rdp_rdpcam_listener_callback*) listener_callback;
    guac_client* client = rdpcam_listener_callback->client;
    guac_rdp_client* rdp_client = (guac_rdp_client*) client->data;
    guac_rdp_rdpcam_plugin* plugin = rdpcam_listener_callback->plugin;

    /* Log new RDPCAM connection */
    const char* ch_name = rdpcam_listener_callback->channel_name ? rdpcam_listener_callback->channel_name : "rdpcam";
    UINT32 channel_id = 0;
    if (plugin && plugin->manager && plugin->manager->GetChannelId)
        channel_id = plugin->manager->GetChannelId(channel);

    guac_client_log(client, GUAC_LOG_DEBUG, "New RDPCAM channel connection (%s) [id=%" PRIu32 "]", ch_name, channel_id);

    /* Ensure there is a device structure for per-channel state. */
    guac_rdpcam_device* device = NULL;

    if (strcasecmp(ch_name, GUAC_RDPCAM_CHANNEL_NAME) != 0) {

        /* Handle device channel connections. */
        device = guac_rdpcam_device_lookup(plugin, ch_name);

        if (!device) {
            device = guac_rdpcam_device_create(plugin, ch_name);
            if (!device) {
                guac_client_log(client, GUAC_LOG_ERROR,
                    "Failed to create RDPCAM device: %s", ch_name);
                *accept = 0;
                return CHANNEL_RC_OK;
            }
...
#endif
                guac_client_log(client, GUAC_LOG_ERROR,
                    "Failed to insert RDPCAM device into registry: %s", ch_name);
            ...
            guac_client_log(client, GUAC_LOG_DEBUG,
                "Created new RDPCAM device: %s", ch_name);
        }
        else {
            pthread_mutex_lock(&device->lock);
            device->ref_count++;
            ...
            guac_client_log(client, GUAC_LOG_DEBUG,
                "Reusing existing RDPCAM device: %s (ref_count=%d)",
    ...
    guac_rdp_rdpcam_channel_callback* rdpcam_channel_callback =
        guac_mem_zalloc(sizeof(guac_rdp_rdpcam_channel_callback));

    /* Init channel callback with data from plugin */
    rdpcam_channel_callback->client = client;
    rdpcam_channel_callback->channel = channel;
    rdpcam_channel_callback->device = device;
    rdpcam_channel_callback->channel_name = ch_name;
    rdpcam_channel_callback->plugin = plugin;
    rdpcam_channel_callback->is_stream_channel = false;
    rdpcam_channel_callback->channel_id = channel_id;
    rdpcam_channel_callback->parent.OnDataReceived = guac_rdp_rdpcam_data;
    rdpcam_channel_callback->parent.OnOpen = guac_rdp_rdpcam_open;
    rdpcam_channel_callback->parent.OnClose = guac_rdp_rdpcam_close;

    /* Accept connection and return callback */
    *accept = 1;
    *channel_callback = (IWTSVirtualChannelCallback*) rdpcam_channel_callback;

    guac_client_log(client, GUAC_LOG_DEBUG, "RDPCAM channel connection established (%s) [id=%" PRIu32 "]",
            ch_name, channel_id);
...
/**
 * Callback which is invoked when the RDPCAM plugin is being initialized and
 * the listener callback structure containing the required callbacks for new
 * connections must be registered.
 *
 * @param plugin
 *     The RDPCAM plugin being initialized.
 *
 * @param manager
 ...
 *     Always zero.
 */
static UINT guac_rdp_rdpcam_initialize(IWTSPlugin* plugin,
        IWTSVirtualChannelManager* manager) {

    /* Allocate control listener */
    guac_rdp_rdpcam_plugin* rdpcam_plugin = (guac_rdp_rdpcam_plugin*) plugin;
    guac_rdp_rdpcam_listener_callback* control_listener =
        guac_mem_zalloc(sizeof(guac_rdp_rdpcam_listener_callback));
    ...
    control_listener->client = rdpcam_plugin->client;
    control_listener->channel_name = GUAC_RDPCAM_CHANNEL_NAME;
    control_listener->plugin = rdpcam_plugin;
    ...
    rdpcam_plugin->manager = manager;
    rdpcam_plugin->enumerator_channel = NULL;
...
/**
 * Callback which is invoked when all connections to the RDPCAM plugin
 * have closed and the plugin is being unloaded.
 *
 * @param plugin
 *     The RDPCAM plugin being unloaded.
 *
 * @return
 *     Always zero.
 */
static UINT guac_rdp_rdpcam_terminated(IWTSPlugin* plugin) {
    ...
    if (rdpcam_plugin->devices != NULL) {
        /* Iterate through channel slots instead of using broken HashTable_GetKeys */
        for (unsigned int check_idx = 0; check_idx < 100; check_idx++) {
            char channel_name[64];
            snprintf(channel_name, sizeof(channel_name), "RDCamera_Device_%u", check_idx);

            guac_rdpcam_device* dev = (guac_rdpcam_device*)
                HashTable_GetItemValue(rdpcam_plugin->devices, channel_name);
            if (dev)
                guac_rdpcam_device_destroy(dev);
        }
        HashTable_Free(rdpcam_plugin->devices);
    ...
    if (rdpcam_plugin->device_id_map != NULL) {
        HashTable_Free(rdpcam_plugin->device_id_map);
        rdpcam_plugin->device_id_map = NULL;
    }

    guac_client_log(rdpcam_plugin->client, GUAC_LOG_DEBUG, 
        "RDPCAM plugin terminated - all devices destroyed");
...
/**
 * Creates a new RDPCAM device structure for multi-device support.
 * This allocates and initializes all per-device state including the stream
 * and prepares for thread creation.
 *
 * @param plugin
 *     The RDPCAM plugin instance.
 *
 * @param device_name
...
static guac_rdpcam_device* guac_rdpcam_device_create(
    ...
    guac_rdpcam_device* device = guac_mem_zalloc(sizeof(guac_rdpcam_device));
    if (!device) {
        guac_client_log(plugin->client, GUAC_LOG_ERROR,
            "Failed to allocate RDPCAM device structure");
        return NULL;
    }
    ...
    if (rdp_client && sscanf(device_name, "RDCamera_Device_%u", &device_index) == 1) {
        ...
        guac_rwlock_acquire_read_lock(&(rdp_client->lock));
        if (device_index < rdp_client->rdpcam_device_caps_count) {
            guac_rdp_rdpcam_device_caps* caps = &rdp_client->rdpcam_device_caps[device_index];
            if (caps->device_id && caps->device_id[0] != '\0') {
                ...
                if (device->browser_device_id) {
                    ...
                    guac_client_log(plugin->client, GUAC_LOG_DEBUG,
                            "RDPCAM device %s mapped to browser device ID: %s",
    ...
    /* Always create a fresh per-device stream for each device.
     * Note: rdp_client->rdpcam_stream is used as a pointer to the active device's stream
     * for the browser to push frames to. It should NOT be reused by new devices,
     * as doing so would steal the stream from an already-active device. */
    device->stream = guac_rdpcam_create(plugin->client);
    if (!device->stream) {
        guac_client_log(plugin->client, GUAC_LOG_ERROR,
            "Failed to create per-device stream for %s", device_name);
    ...
    guac_client_log(plugin->client, GUAC_LOG_DEBUG,
            "RDPCAM stream created for device: %s",
            device_name);
    ...
    if (pthread_mutex_init(&device->lock, NULL) != 0) {
        ...
        guac_rdpcam_destroy(device->stream);
        guac_mem_free(device->device_name);
        guac_mem_free(device);
        return NULL;
    }
    ...
    if (pthread_create(&device->dequeue_thread, NULL, guac_rdp_rdpcam_dequeue_thread, device) != 0) {
        guac_client_log(plugin->client, GUAC_LOG_ERROR,
...
/**
 * Destroys an RDPCAM device structure and frees all associated resources.
...
static void guac_rdpcam_device_destroy(guac_rdpcam_device* device) {
    ...
    guac_client* client = device->stream ? device->stream->client : NULL;
    ...
    if (device->dequeue_thread_started) {
        ...
        if (rc != 0 && client) {
            guac_client_log(client, GUAC_LOG_WARNING,
                "RDPCAM dequeue thread join failed for device %s (rc=%d)",
    ...
    /* Destroy per-device stream */
    if (device->stream) {
        if (rdp_client && rdp_client->rdpcam_stream == device->stream)
            rdp_client->rdpcam_stream = NULL;
        if (client) {
            guac_client_log(client, GUAC_LOG_DEBUG,
                "RDPCAM destroying stream for device: %s",
...
static guac_rdpcam_device* guac_rdpcam_device_lookup(
    ...
    return (guac_rdpcam_device*) HashTable_GetItemValue(
...
static guac_rdp_rdpcam_device_caps* guac_rdp_rdpcam_get_device_caps(
    ...
    if (device_index < rdp_client->rdpcam_device_caps_count)
        return &rdp_client->rdpcam_device_caps[device_index];
 ...
 */
void guac_rdp_rdpcam_send_device_notifications(
        guac_rdp_rdpcam_plugin* plugin, guac_client* client,
    ...
    if (device_count == 0) {
        guac_client_log(client, GUAC_LOG_DEBUG,
                "RDPCAM no devices to announce");
        return;
    }

    guac_client_log(client, GUAC_LOG_DEBUG,
            "RDPCAM sending DeviceAddedNotification for %u device(s)", device_count);
    ...
    for (unsigned int i = 0; i < device_count; i++) {
        ...
        if (rs && rdpcam_build_device_added(rs, device_name, channel_name)) {
            ...
            guac_rdp_rdpcam_log_stream(client, "TX", "RDCamera_Device_Enumerator", enum_channel_id, rs);
            pthread_mutex_lock(&(rdp_client->message_lock));
...
/**
 * Entry point for RDPCAM dynamic virtual channel.
 */
UINT DVCPluginEntry(IDRDYNVC_ENTRY_POINTS* pEntryPoints) {
    ...
    guac_rdp_rdpcam_plugin* rdpcam_plugin = (guac_rdp_rdpcam_plugin*)
        pEntryPoints->GetPlugin(pEntryPoints, GUAC_RDPCAM_PLUGIN_NAME);

    /* If no such plugin allocated, allocate and register it now */
    if (rdpcam_plugin == NULL) {

        /* Init plugin callbacks and data */
        rdpcam_plugin = guac_mem_zalloc(sizeof(guac_rdp_rdpcam_plugin));
        rdpcam_plugin->parent.Initialize = guac_rdp_rdpcam_initialize;
        rdpcam_plugin->parent.Terminated = guac_rdp_rdpcam_terminated;
        rdpcam_plugin->client = client;
        rdpcam_plugin->version_negotiated = false;

        /* Store plugin reference in rdp_client for access from callbacks */
        guac_rdp_client* rdp_client = (guac_rdp_client*) client->data;
        if (rdp_client) {
            rdp_client->rdpcam_plugin = rdpcam_plugin;
            /* Register immediate caps notify callback */
            rdp_client->rdpcam_caps_notify = guac_rdp_rdpcam_caps_notify;
        }

        /* Register plugin for later retrieval */
        pEntryPoints->RegisterPlugin(pEntryPoints, GUAC_RDPCAM_PLUGIN_NAME,
                (IWTSPlugin*) rdpcam_plugin);

        guac_client_log(client, GUAC_LOG_DEBUG, "RDPCAM plugin loaded.");
    }
