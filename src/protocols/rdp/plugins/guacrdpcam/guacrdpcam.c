...
void guac_rdp_rdpcam_caps_notify(guac_client* client) {
    ...
    if (rdp_client->rdpcam_caps_updated && rdp_client->rdpcam_device_caps_count > 0) {
        ...
        guac_client_log(client, GUAC_LOG_DEBUG,
                "RDPCAM sent device notifications via immediate caps notify");
    }
                    ...
                    if (plugin && plugin->devices) {
                        ...
                        for (int i = 0; i < count; i++) {
                            guac_rdpcam_device* old_device =
                                (guac_rdpcam_device*) HashTable_GetItemValue(plugin->devices, (void*) keys[i]);
...
static UINT guac_rdp_rdpcam_terminated(IWTSPlugin* plugin) {
    ...
    if (rdpcam_plugin->devices != NULL) {
        ...
        ULONG_PTR* keys = NULL;
        size_t count = HashTable_GetKeys(rdpcam_plugin->devices, &keys);
        for (size_t i = 0; i < count; i++) {
            void* key = (void*) keys[i];
            guac_rdpcam_device* dev = (guac_rdpcam_device*) HashTable_GetItemValue(rdpcam_plugin->devices, key);
            if (dev)
