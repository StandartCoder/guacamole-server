...
BOOL rdpcam_write_sample_response_header(wStream* s,
    ...
    /* FreeRDP/MS-RDPCAM SampleResponse header:
    ...
    Stream_Write_UINT8(s, RDPCAM_PROTO_VERSION);
    Stream_Write_UINT8(s, RDPCAM_MSG_SAMPLE_RESPONSE);
...
BOOL rdpcam_build_version_request(wStream* s) {
    ...
    Stream_Write_UINT8(s, RDPCAM_PROTO_VERSION);
    Stream_Write_UINT8(s, RDPCAM_MSG_SELECT_VERSION_REQUEST);
    return TRUE;
}
...
BOOL rdpcam_build_version_response(wStream* s) {
    ...
    if (!Stream_EnsureRemainingCapacity(s, 2)) return FALSE;
    Stream_Write_UINT8(s, RDPCAM_PROTO_VERSION);
    Stream_Write_UINT8(s, RDPCAM_MSG_SELECT_VERSION_RESPONSE);
    return TRUE;
}
...
BOOL rdpcam_build_device_added(wStream* s, const char* device_name,
    ...
    Stream_Write_UINT8(s, RDPCAM_PROTO_VERSION);
    Stream_Write_UINT8(s, RDPCAM_MSG_DEVICE_ADDED_NOTIFICATION);
...
BOOL rdpcam_build_success_response(wStream* s) {
    ...
    if (!Stream_EnsureRemainingCapacity(s, 2)) return FALSE;
    Stream_Write_UINT8(s, RDPCAM_PROTO_VERSION);
    Stream_Write_UINT8(s, RDPCAM_MSG_SUCCESS_RESPONSE);
    return TRUE;
}
...
BOOL rdpcam_build_stream_list(wStream* s, const rdpcam_stream_desc* streams, size_t count) {
    ...
    Stream_Write_UINT8(s, RDPCAM_PROTO_VERSION);
    Stream_Write_UINT8(s, RDPCAM_MSG_STREAM_LIST_RESPONSE);
...
BOOL rdpcam_build_media_type_list(wStream* s, const rdpcam_media_type_desc* media_types,
    ...
    Stream_Write_UINT8(s, RDPCAM_PROTO_VERSION);
    Stream_Write_UINT8(s, RDPCAM_MSG_MEDIA_TYPE_LIST_RESPONSE);
...
BOOL rdpcam_build_current_media_type(wStream* s, const rdpcam_media_type_desc* media_type) {
    ...
    Stream_Write_UINT8(s, RDPCAM_PROTO_VERSION);
    Stream_Write_UINT8(s, RDPCAM_MSG_CURRENT_MEDIA_TYPE_RESPONSE);
...
BOOL rdpcam_build_sample_error_response(wStream* s, uint8_t streamIndex) {
    ...
    Stream_Write_UINT8(s, RDPCAM_PROTO_VERSION);
    Stream_Write_UINT8(s, RDPCAM_MSG_SAMPLE_ERROR_RESPONSE);
...
BOOL rdpcam_build_device_removed(wStream* s, const char* channel_name) {
    ...
    Stream_Write_UINT8(s, RDPCAM_PROTO_VERSION);
    Stream_Write_UINT8(s, RDPCAM_MSG_DEVICE_REMOVED_NOTIFICATION);
    Stream_Write(s, channel_name, ch_len);
    return TRUE;
}
