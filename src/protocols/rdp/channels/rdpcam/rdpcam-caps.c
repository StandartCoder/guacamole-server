...
/**
 * Parses RDPCAM capabilities string and updates the RDP client's device
 * capabilities. This is a shared implementation used by both the initial
 * capabilities callback and the update callback.
 *
 * @param client
 *     The guac_client instance.
 *
 * @param rdp_client
 *     The RDP client data (must have write lock held).
 *
 * @param value
 *     The capability string to parse. Must not be NULL.
 *
 * @param is_update
 *     Whether this is an update (true) or initial capabilities (false).
 *     Affects log messages only.
 *
 * @return
 *     The number of devices parsed, or 0 on error.
 */
static unsigned int guac_rdp_rdpcam_parse_capabilities(
        guac_client* client, guac_rdp_client* rdp_client,
        const char* value, bool is_update) {
    ...
    /* Empty string simply clears capabilities */
    if (len == 0) {
        guac_mem_free(copy);
        return 0;
    }
    ...
    while (device_entry && device_count < GUAC_RDP_RDPCAM_MAX_DEVICES) {
        ...
        *colon_pos = '\0';
        ...
        if (format_count > 0) {
            device_count++;
            guac_client_log(client, GUAC_LOG_DEBUG,
                    "RDPCAM %s device %u: id='%s', name='%s', formats=%u",
...
int guac_rdp_rdpcam_capabilities_callback(guac_user* user,
    ...
    bool is_update = (name && strcmp(name, GUAC_RDPCAM_ARG_CAPABILITIES_UPDATE) == 0);

    guac_rwlock_acquire_write_lock(&(rdp_client->lock));

    unsigned int device_count = guac_rdp_rdpcam_parse_capabilities(
            client, rdp_client, value, is_update);
    ...
    guac_client_log(client, GUAC_LOG_DEBUG,
            "RDPCAM %s capability data processed (%u devices), notifying plugin",
            is_update ? "update" : "initial", device_count);
