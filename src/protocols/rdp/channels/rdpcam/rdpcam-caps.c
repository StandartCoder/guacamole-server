...
size_t guac_rdp_rdpcam_sanitize_device_name(const char* name, char* sanitized, size_t len) {
...
int guac_rdp_rdpcam_capabilities_callback(guac_user* user,
    ...
    for (unsigned int i = 0; i < rdp_client->rdpcam_device_caps_count; i++) {
    ...
    rdp_client->rdpcam_device_caps_count = 0;
    ...
    if (!device_entry) {
        guac_client_log(client, GUAC_LOG_WARNING,
                "RDPCAM received capabilities in invalid format (expected semicolon-separated device list)");
        guac_mem_free(copy);
        return 0;
    }

    while (device_entry && device_count < GUAC_RDP_RDPCAM_MAX_DEVICES) {
        guac_rdp_rdpcam_device_caps* caps = &rdp_client->rdpcam_device_caps[device_count];
        ...
        if (!pipe_pos) {
            guac_client_log(client, GUAC_LOG_DEBUG,
                    "RDPCAM skipping device entry without pipe separator: '%s'", device_entry);
        ...
        *pipe_pos = '\0';
        ...
        if (!device_info || !*device_info) {
            guac_client_log(client, GUAC_LOG_DEBUG,
                    "RDPCAM skipping device entry without device info");
            device_entry = strtok_r(NULL, ";", &device_saveptr);
            continue;
        }
        ...
        if (!colon_pos) {
            guac_client_log(client, GUAC_LOG_WARNING,
                    "RDPCAM skipping device entry without device ID (format: DEVICE_ID:DEVICE_NAME required): '%s'",
                    device_info);
            device_entry = strtok_r(NULL, ";", &device_saveptr);
            continue;
        }
        ...
        *colon_pos = '\0';
        ...
        if (!device_id_parsed || !*device_id_parsed) {
            guac_client_log(client, GUAC_LOG_WARNING,
                    "RDPCAM skipping device entry with empty device ID");
            device_entry = strtok_r(NULL, ";", &device_saveptr);
            continue;
        }
        ...
        if (!caps->device_id) {
            guac_client_log(client, GUAC_LOG_ERROR,
                    "RDPCAM failed to allocate device ID string");
            device_entry = strtok_r(NULL, ";", &device_saveptr);
            continue;
        }
        ...
        if (device_name_parsed && *device_name_parsed) {
            char sanitized[256];
            size_t sanitized_len = guac_rdp_rdpcam_sanitize_device_name(
        ...
        while (format_token && format_count < GUAC_RDP_RDPCAM_MAX_FORMATS) {
            /* Trim whitespace */
            ...
            if (parsed >= 3 && width && height && fps_num) {
                guac_rdp_rdpcam_format* fmt = &caps->formats[format_count++];
                fmt->width = width;
                fmt->height = height;
                fmt->fps_num = fps_num;
                ...
                guac_client_log(client, GUAC_LOG_DEBUG,
                        "RDPCAM ignored unparseable format entry: '%s'", format_token);
            }
        ...
        if (format_count > 0) {
            device_count++;
            guac_client_log(client, GUAC_LOG_DEBUG,
                    "RDPCAM device %u: id='%s', name='%s', formats=%u",
                    device_count - 1,
                    caps->device_id,
            ...
            guac_client_log(client, GUAC_LOG_WARNING,
                    "RDPCAM skipping device '%s' (id='%s') with no valid formats",
    ...
    rdp_client->rdpcam_device_caps_count = device_count;
    
    /* Set flag to notify plugin that capabilities have been updated. */
    rdp_client->rdpcam_caps_updated = 1;
    ...
    if (rdp_client->rdpcam_caps_notify)
        rdp_client->rdpcam_caps_notify(client);

    guac_rwlock_release_lock(&(rdp_client->lock));

    guac_client_log(client, GUAC_LOG_DEBUG,
            "RDPCAM capabilities updated (%u devices), notifying plugin", device_count);

    guac_mem_free(copy);
    return 0;
}

int guac_rdp_rdpcam_capabilities_update_callback(guac_user* user,
        const char* mimetype, const char* name, const char* value, void* data) {
    ...
    guac_client_log(client, GUAC_LOG_DEBUG,
            "RDPCAM capability update received: %zu bytes", strlen(value));

    /* Handle empty capability string (all cameras disabled) */
    if (strlen(value) == 0) {
        guac_rwlock_acquire_write_lock(&(rdp_client->lock));
        ...
        guac_client_log(client, GUAC_LOG_DEBUG,
                "RDPCAM all cameras disabled, notifying plugin");
        return 0;
    }

    /* Reuse the same parsing logic as the initial capabilities callback */
    size_t len = strlen(value);
    char* copy = guac_mem_alloc(len + 1);
    if (!copy)
        return 0;
    ...
    /* Parse multi-device capabilities (same format as initial capabilities) */
    unsigned int device_count = 0;
    char* device_saveptr = NULL;
    char* device_entry = strtok_r(copy, ";", &device_saveptr);
    ...
    if (!device_entry) {
        guac_client_log(client, GUAC_LOG_WARNING,
                "RDPCAM capability update in invalid format (expected semicolon-separated device list)");
        guac_mem_free(copy);
        guac_rwlock_release_lock(&(rdp_client->lock));
        return 0;
    }
    ...
    while (device_entry && device_count < GUAC_RDP_RDPCAM_MAX_DEVICES) {
        ...
        *pipe_pos = '\0';
        ...
        if (!colon_pos) {
            guac_client_log(client, GUAC_LOG_WARNING,
                    "RDPCAM skipping device entry without device ID: '%s'", device_info);
            device_entry = strtok_r(NULL, ";", &device_saveptr);
            continue;
        }
        ...
        *colon_pos = '\0';
        ...
        /* Only add device if it has valid formats */
        if (format_count > 0) {
            device_count++;
            guac_client_log(client, GUAC_LOG_DEBUG,
                    "RDPCAM update device %u: id='%s', name='%s', formats=%u",
                    device_count - 1,
                    caps->device_id,
                    caps->device_name ? caps->device_name : "(none)",
                    format_count);
    ...
    /* Notify plugin to process the update (compare old vs new and add/remove devices) */
    if (rdp_client->rdpcam_caps_notify)
        rdp_client->rdpcam_caps_notify(client);
    ...
    guac_client_log(client, GUAC_LOG_DEBUG,
            "RDPCAM capability update processed (%u devices), notifying plugin", device_count);
