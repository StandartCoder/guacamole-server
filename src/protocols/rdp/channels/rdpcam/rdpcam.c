...
/**
 * Per-stream reassembly state for RDPCAM frames. Handles fragmentation
 * across arbitrary Guacamole blob boundaries. One instance is attached to
 * the Guacamole stream via stream->data.
 */
typedef struct guac_rdp_rdpcam_stream_state {

    /* Header assembly */
    uint8_t header_buf[sizeof(guac_rdpcam_frame_header)];
...
} guac_rdp_rdpcam_stream_state;

/**
 * Parses the given RDPCAM mimetype, validating that it's the expected format.
 *
 * @param mimetype
 *     The RDPCAM mimetype to parse.
 *
 * @return
 *     Zero if the given mimetype is valid, non-zero otherwise.
 */
static int guac_rdp_rdpcam_parse_mimetype(const char* mimetype) {

    /* Validate RDPCAM H.264 mimetype */
    if (strcmp(mimetype, "application/rdpcam+h264") == 0)
        return 0;
...
int guac_rdp_rdpcam_video_handler(guac_user* user, guac_stream* stream,
        char* mimetype) {
    ...
    if (guac_rdp_rdpcam_parse_mimetype(mimetype)) {
    ...
    /* Init stream data */
    stream->blob_handler = guac_rdp_rdpcam_blob_handler;
    stream->end_handler = guac_rdp_rdpcam_end_handler;

    /* Initialize per-stream reassembly state */
    if (!stream->data) {
        guac_rdp_rdpcam_stream_state* st = guac_mem_zalloc(sizeof(guac_rdp_rdpcam_stream_state));
        stream->data = st;
    }

    /* Associate stream with RDPCAM stream */
    if (rdp_client->rdpcam_stream) {
        ...
        guac_user_log(user, GUAC_LOG_WARNING, "RDPCAM stream not available");
        guac_protocol_send_ack(user->socket, stream, "RDPCAM not available",
...
int guac_rdp_rdpcam_blob_handler(guac_user* user, guac_stream* stream,
        void* data, int length) {

    guac_client* client = user->client;
    guac_rdp_client* rdp_client = (guac_rdp_client*) client->data;

    /* If stream not available, fail */
    if (!rdp_client->rdpcam_stream) {
        guac_protocol_send_ack(user->socket, stream, "RDPCAM not available",
                GUAC_PROTOCOL_STATUS_SERVER_ERROR);
        guac_socket_flush(user->socket);
    ...
    guac_rdp_rdpcam_stream_state* st = (guac_rdp_rdpcam_stream_state*) stream->data;
    if (!st) {
        st = guac_mem_zalloc(sizeof(guac_rdp_rdpcam_stream_state));
        stream->data = st;
    }
    ...
    while (remaining > 0) {
        ...
        if (st->frame_expected == 0) {
            size_t need = sizeof(guac_rdpcam_frame_header) - st->header_received;
            ...
            if (st->header_received < sizeof(guac_rdpcam_frame_header)) {
                /* Need more data to finish header */
                break;
            }

            /* Header complete: validate and start frame assembly */
            const guac_rdpcam_frame_header* hdr = (const guac_rdpcam_frame_header*) st->header_buf;

            /* Basic sanity checks to avoid pathological allocations */
            if (hdr->version != 1 || hdr->payload_len > GUAC_RDPCAM_MAX_FRAME_SIZE) {
                guac_user_log(user, GUAC_LOG_WARNING,
                        "RDPCAM invalid header (version=%u, payload_len=%u) - discarding corrupted data (likely camera switch in progress)",
            ...
            st->frame_expected = sizeof(guac_rdpcam_frame_header) + (size_t) hdr->payload_len;
            st->frame_buf = guac_mem_alloc(st->frame_expected);
            if (!st->frame_buf) {
                guac_user_log(user, GUAC_LOG_ERROR, "RDPCAM failed to allocate reassembly buffer: %zu bytes", st->frame_expected);
            ...
            memcpy(st->frame_buf, st->header_buf, sizeof(guac_rdpcam_frame_header));
            st->frame_received = sizeof(guac_rdpcam_frame_header);
        ...
        if (st->frame_received == st->frame_expected) {
            /* We have a full frame: push to stream (failures are tracked in periodic stats) */
            guac_rdpcam_push(rdp_client->rdpcam_stream, st->frame_buf, st->frame_expected);
            ...
            guac_user_log(user, GUAC_LOG_TRACE,
                    "RDPCAM partial frame: %zu/%zu bytes", st->frame_received, st->frame_expected);
        }
    }
...
int guac_rdp_rdpcam_end_handler(guac_user* user, guac_stream* stream) {
...
void guac_rdp_rdpcam_load_plugin(rdpContext* context) {

    guac_client* client = ((rdp_freerdp_context*) context)->client;
    char client_ref[GUAC_RDP_PTR_STRING_LENGTH];

    /* Add "guacrdpcam" plugin (loads libguacrdpcam-client.so) */
    guac_rdp_ptr_to_string(client, client_ref);
    guac_freerdp_dynamic_channel_collection_add(context->settings, "guacrdpcam", client_ref, NULL);
