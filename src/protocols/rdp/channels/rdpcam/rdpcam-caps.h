...
#ifndef GUAC_RDP_CHANNELS_RDPCAM_CAPS_H
#define GUAC_RDP_CHANNELS_RDPCAM_CAPS_H
 ...
 */
#define GUAC_RDPCAM_ARG_CAPABILITIES "rdpcam-capabilities"
 ...
 */
#define GUAC_RDPCAM_ARG_CAPABILITIES_UPDATE "rdpcam-capabilities-update"

/**
 * Maximum number of RDPCAM formats remembered from the browser.
 */
#define GUAC_RDP_RDPCAM_MAX_FORMATS 16

/**
 * Maximum number of camera devices that can be redirected simultaneously.
 */
#define GUAC_RDP_RDPCAM_MAX_DEVICES 8

/**
 * Describes a single camera format (resolution + frame rate) reported by the
 * browser.
 */
typedef struct guac_rdp_rdpcam_format {
    /**
...
} guac_rdp_rdpcam_format;

/**
 * Per-device camera capabilities reported by the browser.
 */
typedef struct guac_rdp_rdpcam_device_caps {
    /**
     ...
     */
    guac_rdp_rdpcam_format formats[GUAC_RDP_RDPCAM_MAX_FORMATS];

    /**
     * Number of valid entries within formats array.
     */
    unsigned int format_count;
} guac_rdp_rdpcam_device_caps;
 ...
 *
 * @param name
 *     The name of the argument (should be "rdpcam-capabilities").
 *
 * @param value
 ...
 *
 * @param name
 *     The name of the argument (should be "rdpcam-capabilities-update").
 *
 * @param value
